...
/* ════════════════════════════════════════════
   Orion Spacecraft (extracted NASA AROW GLB)
   ════════════════════════════════════════════ */
...
function buildOrionPlaceholder(): OrionCraft {
  ...
  const engineGlow = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0x3ee0ff, transparent: true, opacity: 0.25 })
  );
...
function loadOrionModel(
  ...
  gltfLoader.load(
    ...
    (gltf) => {
      ...
      /* Model Y-up → scene Z-forward: rotate so CM (top) faces +Z (travel direction) */
...
export function createOrbitScene(canvas: HTMLCanvasElement, clock: OrbitClock): OrbitSceneApi {
  ...
  const posMarker = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0xff6b4a, transparent: true, opacity: 0.15 })
  );
  scene.add(posMarker);
